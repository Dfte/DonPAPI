        ...
        if directory_list is not None:
            for item in directory_list:
                if item.get_longname() not in self.false_positive:
                    ...
                    # Stores file in $DPP_LOOT/.donpapi/loot/TARGET/[...]
                    local_filepath = os.path.join(self.context.target_output_dir, *(new_path.split('\\')))
                    ...
                    # Stores file in $DPP_LOOT/.donpapi/loot/COLLECTOR/
                    os.makedirs(f"{self.context.global_output_dir}/{self.tag}/", exist_ok=True)
