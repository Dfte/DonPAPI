...
class Certificates:
    ...
    def print_and_store(self, certificate, cert_username, filename) -> None:
        absolute_local_filepath = os.path.join(self.context.target_output_dir, filename)
        dump_file_to_loot_directories(absolute_local_filepath, certificate.pfx)
        collector_dir_local_filepath = os.path.join(self.context.global_output_dir, self.tag, filename)
